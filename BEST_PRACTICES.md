...
- **C — VEX type-prefixes and ambiguous signatures fail quietly or hard.** There is no `w@` prefix (`@w` is a vector → use `v@w`); `colormap(...).r` and `(int)prim(...)` are ambiguous and abort the compile. Assign `point()`/`prim()` to a **typed variable** before `setprimattrib`/`setpointattrib` — `int st = point(0,'stage',@ptnum); setprimattrib(0,'stage',pr,st)` compiles; the inline call is type-ambiguous and aborts. A value stored as an **int** primvar samples as **0** in a float Karma AOV — `float()`-cast anything that must render as an AOV. In `rotate()` the int axis arg is a **bitmask** (XAXIS=1, YAXIS=2, ZAXIS=4), not an index 0/1/2 — pass a vector axis (`{0,1,0}`). After setting any wrangle, run `find_error_nodes`.
